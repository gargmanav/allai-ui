...
// ============================================================================
// 4-BUTTON CONTRACTOR RESPONSE SYSTEM
// ============================================================================

// Helper: add business days to a date (skips weekends)
...
// Button 1: Send Quote + Availability
router.post('/respond/quote', requireAuth, requireRole('contractor'), async (req: AuthenticatedRequest, res) => {
  try {
    const contractorUserId = req.user!.id;
    const { caseId, quotedPrice, availableStartDate, availableEndDate, estimatedDays, notes } = req.body;

    if (!caseId || !quotedPrice) {
      return res.status(400).json({ error: 'Case ID and quoted price are required' });
    }
    ...
    const [response] = await db.insert(contractorResponses).values({
      caseId,
      contractorUserId,
      orgId: caseRecord.orgId,
      responseType: 'quote',
      ...
      availableEndDate: availableEndDate ? new Date(availableEndDate) : null,
    ...
    const policy = await db.query.approvalPolicies.findFirst({
      where: and(eq(approvalPolicies.orgId, caseRecord.orgId), eq(approvalPolicies.isActive, true)),
    });
    const involvementMode = policy?.involvementMode || 'balanced';
    const autoApproveCostLimit = policy?.autoApproveCostLimit ? parseFloat(String(policy.autoApproveCostLimit)) : 500;
    ...
    const isFavorite = await db.query.favoriteContractors.findFirst({
      where: and(eq(favoriteContractors.orgId, caseRecord.orgId), eq(favoriteContractors.contractorUserId, contractorUserId)),
    });
    const isTrusted = trustedIds.includes(contractorUserId) || !!isFavorite;
    const quoteAmount = parseFloat(String(quotedPrice));

    const shouldAutoApprove = involvementMode === 'hands-off' && isTrusted && quoteAmount <= autoApproveCostLimit;

    const newStatus = shouldAutoApprove ? 'Confirmed' : 'Awaiting Approval';

    await db.update(smartCases).set({
      status: newStatus,
      assignedContractorId: shouldAutoApprove ? contractorUserId : caseRecord.assignedContractorId,
      quotedPrice: String(quotedPrice),
      contractorResponseType: 'quote',
      updatedAt: new Date(),
    }).where(eq(smartCases.id, caseId));
    ...
    await db.insert(caseEvents).values({
      ...
      description: shouldAutoApprove
        ? `Quote of $${quoteAmount.toFixed(2)} auto-approved (hands-off mode, trusted contractor, under threshold)`
        : `Quote of $${quoteAmount.toFixed(2)} submitted, awaiting landlord approval`,
      metadata: { responseId: response.id, responseType: 'quote', quotedPrice: quoteAmount, autoApproved: shouldAutoApprove },
    });

    res.json({ success: true, autoApproved: shouldAutoApprove, status: newStatus, responseId: response.id });
  } catch (error) {
...
// Button 2: Request Diagnostic Visit + Fee + Availability + Quote Turnaround
router.post('/respond/diagnostic', requireAuth, requireRole('contractor'), async (req: AuthenticatedRequest, res) => {
  try {
    const contractorUserId = req.user!.id;
    const { caseId, diagnosticFee, availableStartDate, availableEndDate, quoteTurnaroundDays, notes } = req.body;
    ...
    const fee = diagnosticFee ? parseFloat(diagnosticFee) : 0;
    const turnaroundDays = quoteTurnaroundDays ? parseInt(quoteTurnaroundDays) : 3;

    const [response] = await db.insert(contractorResponses).values({
      caseId,
      contractorUserId,
      orgId: caseRecord.orgId,
      responseType: 'diagnostic_visit',
      diagnosticFee: fee > 0 ? String(fee) : null,
      availableStartDate: availableStartDate ? new Date(availableStartDate) : null,
      availableEndDate: availableEndDate ? new Date(availableEndDate) : null,
      quoteTurnaroundDays: turnaroundDays,
      notes: notes || null,
      status: 'submitted',
    }).returning();
    ...
    await db.update(smartCases).set({
      ...
      isDiagnosticPhase: true,
      diagnosticFee: fee > 0 ? String(fee) : null,
      quoteTurnaroundDays: turnaroundDays,
      contractorResponseType: 'diagnostic_visit',
      assignedContractorId: caseRecord.assignedContractorId || contractorUserId,
      updatedAt: new Date(),
    }).where(eq(smartCases.id, caseId));
    ...
    await db.insert(caseEvents).values({
      caseId,
      type: 'contractor_response',
      description: `Diagnostic visit requested${fee > 0 ? ` ($${fee.toFixed(2)} fee)` : ' (no fee)'}. Available: ${availableStartDate || 'TBD'}. Quote turnaround: ${turnaroundDays} business days.`,
      metadata: { responseId: response.id, responseType: 'diagnostic_visit', diagnosticFee: fee, turnaroundDays },
    });

    res.json({ success: true, status: 'Awaiting Approval', responseId: response.id });
  } catch (error) {
    console.error('Error submitting diagnostic request:', error);
    res.status(500).json({ error: 'Failed to submit diagnostic visit request' });
  }
});

// Button 3: Need More Info (questions for landlord/homeowner)
router.post('/respond/need-info', requireAuth, requireRole('contractor'), async (req: AuthenticatedRequest, res) => {
  try {
    const contractorUserId = req.user!.id;
    const { caseId, questions, notes } = req.body;

    if (!caseId || (!questions?.length && !notes)) {
      return res.status(400).json({ error: 'Case ID and at least one question or note are required' });
    }
    ...
    const questionList = Array.isArray(questions) ? questions.slice(0, 3) : [];
    ...
    const [response] = await db.insert(contractorResponses).values({
      ...
      questions: questionList.length > 0 ? questionList : null,
      notes: notes || null,
      status: 'awaiting_reply',
    }).returning();

    await db.update(smartCases).set({
      contractorResponseType: 'need_more_info',
      updatedAt: new Date(),
    }).where(eq(smartCases.id, caseId));
    ...
    await db.insert(caseEvents).values({
      caseId,
      type: 'contractor_response',
      description: `Contractor needs more information: ${questionList.length} question(s) asked`,
      metadata: { responseId: response.id, responseType: 'need_more_info', questions: questionList },
    });
...
// Button 4: Pass / Decline
router.post('/respond/pass', requireAuth, requireRole('contractor'), async (req: AuthenticatedRequest, res) => {
  try {
    const contractorUserId = req.user!.id;
    const { caseId, reason } = req.body;
    ...
    await db.insert(contractorDismissedCases).values({
      contractorUserId,
      caseId,
      reason: reason || 'Passed via response buttons',
    }).onConflictDoNothing();

    if (caseRecord.assignedContractorId === contractorUserId) {
      await db.update(smartCases).set({
        assignedContractorId: null,
        status: 'Awaiting Contractor',
        contractorResponseType: null,
        updatedAt: new Date(),
      }).where(eq(smartCases.id, caseId));
    }

    const totalPasses = await db.select({ count: sql<number>`count(*)` })
      .from(contractorResponses)
      .where(and(eq(contractorResponses.caseId, caseId), eq(contractorResponses.responseType, 'pass')));

    const passCount = Number(totalPasses[0]?.count || 0);

    await db.insert(caseEvents).values({
      caseId,
      type: 'contractor_response',
      description: `Contractor declined${reason ? `: ${reason}` : ''}. ${passCount >= 3 ? 'Multiple contractors have declined — landlord notified.' : ''}`,
      metadata: { responseId: response.id, responseType: 'pass', passCount, reason },
    });

    res.json({ success: true, passCount, needsReassignment: passCount >= 3 });
  } catch (error) {
    console.error('Error submitting pass response:', error);
    res.status(500).json({ error: 'Failed to decline case' });
  }
});

// Get contractor responses for a case
router.get('/responses/:caseId', requireAuth, requireRole('contractor'), async (req: AuthenticatedRequest, res) => {
  try {
    const contractorUserId = req.user!.id;
    const { caseId } = req.params;

    const responses = await db.query.contractorResponses.findMany({
      where: and(
        eq(contractorResponses.caseId, caseId),
        eq(contractorResponses.contractorUserId, contractorUserId)
      ),
...
// Request SLA extension for quote deadline
router.post('/sla-extension', requireAuth, requireRole('contractor'), async (req: AuthenticatedRequest, res) => {
  try {
    const contractorUserId = req.user!.id;
    const { caseId, reason, requestedDeadline } = req.body;

    if (!caseId || !reason || !requestedDeadline) {
      return res.status(400).json({ error: 'Case ID, reason, and requested deadline are required' });
    }
    ...
    if (caseRecord.status !== 'Awaiting Quote') {
      return res.status(400).json({ error: 'Case is not in Awaiting Quote status' });
    }

    const currentExtensions = caseRecord.slaExtensionCount || 0;
    const extensionNumber = currentExtensions + 1;
    const isAutoGranted = extensionNumber === 1;

    const [extension] = await db.insert(slaExtensionRequests).values({
      caseId,
      contractorUserId,
      reason,
      requestedDeadline: new Date(requestedDeadline),
      previousDeadline: caseRecord.quoteSlaDeadline || new Date(),
      extensionNumber,
      status: isAutoGranted ? 'approved' : 'pending',
    }).returning();

    if (isAutoGranted) {
      await db.update(smartCases).set({
        quoteSlaDeadline: new Date(requestedDeadline),
        slaExtensionCount: extensionNumber,
        updatedAt: new Date(),
      }).where(eq(smartCases.id, caseId));
    } else {
      await db.update(smartCases).set({
    ...
    await db.insert(caseEvents).values({
      caseId,
      type: 'sla_extension',
      description: isAutoGranted
        ? `SLA extension #${extensionNumber} auto-granted. New deadline: ${new Date(requestedDeadline).toLocaleDateString()}`
        : `SLA extension #${extensionNumber} requested, awaiting landlord approval. Reason: ${reason}`,
      metadata: { extensionId: extension.id, extensionNumber, isAutoGranted, reason },
    });

    res.json({ success: true, extensionId: extension.id, autoGranted: isAutoGranted, extensionNumber });
  } catch (error) {
    console.error('Error requesting SLA extension:', error);
